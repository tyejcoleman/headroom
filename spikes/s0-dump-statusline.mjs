...
// statusline and appends it to ~/.tokenroom/raw-sample.json so we can verify, on a real
...
//     "command": "node /Users/taikicoleman/Development/tokenroom/spikes/s0-dump-statusline.mjs" }
// Then use Claude Code normally for a few prompts and inspect:
//   tail -3 ~/.tokenroom/raw-sample.json
...
process.stdin.on('end', () => {
  const dir = join(homedir(), '.tokenroom');
  try {
  ...
  try {
    ...
    hud = `tokenroom S0 · ${rl} · ${cw}${pct}`;
  } catch {
    hud = 'tokenroom S0 · captured (stdin not valid JSON)';
  }
