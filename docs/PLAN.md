...
- [~] **T2.15 Armed resume (auto wake at reset).** *(shipped + ARMED LIVE 2026-06-10, ADR-16 written; **REMOVED 2026-07-01, ADR-22** — the 2026-06-15 platform change priced programmatic use (`claude -p`, SDK) into a separate API-billed credit pool, breaking the "use quota that would otherwise expire" economics. The executor (`src/arm.mjs`, launchd plist, `--arm`/`--disarm`, `auto_arm`) is gone; the awareness half — `plan_resume`, HUD countdown, readiness stamps, `headroom resume --clear` — stays (T2.3). Autonomous continuation moves to the separate Conductor package on in-session surfaces.)*
...
- [x] **T2.29 Continuity handoff doc (the living working-doc).** *(shipped 2026-06-17, ADR-18; wording in eval queue)* `checkpoint` (ADR-15) is the terse survival ping; this is the rich, evolving, model-authored **markdown** handoff doc a fresh instance reads to resume fully — mission / current state / progress / exact next steps / key references / decisions / ruled-out / **the user's directives** / improvements / open questions. `handoff` MCP tool (fourth write surface), stored at `~/.headroom/continuity/<session>.md`, re-injected as pointer+digest at SessionStart(source=compact). The skill reframes context-pressure as a write-the-handoff ritual, not a stop sign — addresses the field report that an agent got "tired"/cautious near compaction instead of handing off and continuing. `headroom handoff [--path]` CLI to view. AC: lifecycle unit-tested (save / caps / session-guard / take / render + MCP round-trip + SessionStart re-injection); skill + ctx-band wording eval-gated (ADR-9) before publish.
...
- [x] **T2.28 Continue-until-done rule.** *(shipped 2026-06-13, skill; eval pending ADR-9)*
  ...
  Conductor's job, not headroom's.
