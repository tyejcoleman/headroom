...
- [x] **T2.2 Compaction survival.** *(shipped 2026-06-09)* `PreCompact` hook snapshots ground truth a hook can actually capture — branch, uncommitted files, recent commits, budget state — to `~/.headroom/handoffs/<session>.json` (hooks have no model access, so repo facts beat prose; `customInstructions` injection isn't an official surface, dropped). `SessionStart(source=compact)` re-injects it with staleness + wrong-session guards. AC: unit-tested end-to-end; G2-sim continuity eval run (equipped agents resume with ~19% fewer tool calls on the small fixture; differentiation expected to grow with repo size — see `eval/v2-continuity/results/`). Larger-fixture eval listed as follow-up.
