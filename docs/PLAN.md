...
> (`tokenroom`, plain ESM + `node:test`) instead of the TS/pnpm monorepo — npx-able,
...
- [x] **T0.2 `tokenroom-tap` v0.** Statusline command: parse stdin JSON; extract `rate_limits` (absent → degrade, never crash; out-of-range `used_percentage` incl. epoch leaks → null) and `context_window`; atomically write `~/.tokenroom/state.json` (temp file + rename); render a one-line HUD. AC met: survives malformed/missing/empty stdin (tested); registered by the installer. `--capture` debug flag records raw payloads (subsumes Spike S0).
...
- [x] **T1.1 `tokenroom-mcp`.** stdio MCP server (hand-rolled newline-delimited JSON-RPC, zero-dep), read-only over `state.json`. Tools: `resource_state`, `estimate_remaining`, `fit_check({est_tokens, est_calls})` → `fits | tight | exceeds | defer`. AC met: registered via `claude mcp add` by the installer; no writes, no network; round-trip tested.
- [x] **T1.2 Push injection.** `UserPromptSubmit` hook returns an `additionalContext` stamp — **remaining-first wording** (eval v0 found "X% used" gets misread): `[tokenroom] 5h: 58% left, resets 14:00 · 7d: 85% left · ctx: ~38k tokens before compaction`. AC met: ≤40 tokens (length-tested); silent when stale/missing/disabled (`TOKENROOM_DISABLE=1` or config).
...
- [x] **T1.4 Installer.** `tokenroom install`: sets the statusline command, registers the MCP server, hook, and skill; idempotent; `uninstall` restores any pre-existing statusline from backup. AC met: sandbox-tested (double-install safe; uninstall leaves no trace). npm publish as `tokenroom` pending → then `npx tokenroom install`.
...
- [x] **T2.2 Compaction survival.** *(shipped 2026-06-09)* `PreCompact` hook snapshots ground truth a hook can actually capture — branch, uncommitted files, recent commits, budget state — to `~/.tokenroom/handoffs/<session>.json` (hooks have no model access, so repo facts beat prose; `customInstructions` injection isn't an official surface, dropped). `SessionStart(source=compact)` re-injects it with staleness + wrong-session guards. AC: unit-tested end-to-end; G2-sim continuity eval run (equipped agents resume with ~19% fewer tool calls on the small fixture; differentiation expected to grow with repo size — see `eval/v2-continuity/results/`). Larger-fixture eval run 2026-06-10: honest null on outcome on clean trees, path/speed signal confirmed — see results; v4 needs distractor dirt.
- [x] **T2.3 Reset scheduler.** *(shipped 2026-06-09)* `plan_resume` MCP tool records a deferred-work plan keyed to `resets_at` (the server's one deliberate write surface); HUD shows `⏲ resume HH:MM` countdown, then `✓ deferred ready`; prompt stamps and SessionStart flag readiness after the reset; `tokenroom resume [--clear]` for humans. Interactive resume only — no headless quota burn. AC: full lifecycle covered in `test/continuity.test.mjs`.
- [x] **T2.4 Governor v0 (soft).** *(shipped 2026-06-10)* `mode: performance | ondemand | powersave` shifts WHEN tokenroom speaks, never what it says: band thresholds (perf 10/5 · ondemand 25/10/5 · powersave 40/25/10/5), receipt floors, and the re-stamp throttle all come from the mode profile, read per-event (config change applies without restart — tested). The behavioral lever is the same eval-validated band/stamp machinery (ADR-9 evidence carries over); a dedicated mode A/B eval joins the v4 queue.
- [x] **T2.5 Eval harness + README metrics.** *(shipped 2026-06-10 — `npm run eval` regenerates eval/REPORT.md, failing on missing evidence; README links it.)* Window utilization %, tasks per window, 429 count, wasted tokenroom at reset, compaction-continuity score; naive vs. Tokenroom-equipped comparison chart. AC: reproducible `pnpm eval` run generates the chart.
...
- [x] **T2.7 Pins.** *(shipped 2026-06-10)* `pin_fact` MCP tool (second write surface, ADR-12) + `tokenroom pin|pins|unpin`; pins re-injected verbatim at SessionStart(source=compact); capped (500 chars, 50 pins, 7d TTL). AC: full lifecycle unit-tested incl. TTL expiry and MCP path without state; skill teaches when to pin. *(ADR-9 eval PASSED 2026-06-10 — `eval/v3-wording/results/`.)*
...
- [x] **T2.9 Compaction observability.** *(shipped 2026-06-10)* PostCompact hook + event log (`~/.tokenroom/events.jsonl`, capped); tap detects silent context cliffs (microcompaction fires no hooks) for the same session; next stamp discloses once with the transcript path as recovery route; suppressed when a real compact/clear/session event explains the drop. AC: detect, announce-once, and suppression all unit-tested.
...
- [~] **T2.15 Armed resume (auto wake at reset).** *(shipped + ARMED LIVE 2026-06-10, ADR-16 written; **REMOVED 2026-07-01, ADR-22** — the 2026-06-15 platform change priced programmatic use (`claude -p`, SDK) into a separate API-billed credit pool, breaking the "use quota that would otherwise expire" economics. The executor (`src/arm.mjs`, launchd plist, `--arm`/`--disarm`, `auto_arm`) is gone; the awareness half — `plan_resume`, HUD countdown, readiness stamps, `tokenroom resume --clear` — stays (T2.3). Autonomous continuation moves to the separate Conductor package on in-session surfaces.)*
...
- [x] **T2.29 Continuity handoff doc (the living working-doc).** *(shipped 2026-06-17, ADR-18; wording in eval queue)* `checkpoint` (ADR-15) is the terse survival ping; this is the rich, evolving, model-authored **markdown** handoff doc a fresh instance reads to resume fully — mission / current state / progress / exact next steps / key references / decisions / ruled-out / **the user's directives** / improvements / open questions. `handoff` MCP tool (fourth write surface), stored at `~/.tokenroom/continuity/<session>.md`, re-injected as pointer+digest at SessionStart(source=compact). The skill reframes context-pressure as a write-the-handoff ritual, not a stop sign — addresses the field report that an agent got "tired"/cautious near compaction instead of handing off and continuing. `tokenroom handoff [--path]` CLI to view. AC: lifecycle unit-tested (save / caps / session-guard / take / render + MCP round-trip + SessionStart re-injection); skill + ctx-band wording eval-gated (ADR-9) before publish.
...
- [ ] **T2.16 `tokenroom recap`.** End-of-session / `--week` story from existing data
...
- [ ] **T2.17 `tokenroom drill`.** Compaction preview: render exactly what would survive
...
- [ ] **T2.18 Multi-session watch.** Per-session state retention (state-<session>.json
  alongside last-writer state.json) so `tokenroom watch` shows every live session's
...
- [ ] **T2.19 `tokenroom brief`.** Morning block: deferred-ready, pins, last recap line,
...
- [x] **T2.28 Continue-until-done rule.** *(shipped 2026-06-13, skill; eval pending ADR-9)*
  ...
  anti-timidity arc. Tokenroom INFLUENCES the stop decision (its lane); it does NOT drive
  the turn loop — continuous operation within a session is the harness's / Keyoku's /
  Conductor's job, not tokenroom's.

## Phase 2.6 — Self-evolving harness (propose-only; docs/EVOLVING-HARNESS.md, SUGGEST.md)

- [~] **T2.22 `tokenroom suggest` miner (v0).** *(built + shipped in 0.3.2, then DEFERRED 2026-06-13 — code removed, design kept in docs/SUGGEST.md; re-enter post-adoption when there is real telemetry to mine. Thinness fitness function applied to ourselves.)* Deterministic
...
- [x] **T2.23 Friction feed contract.** *(satisfied by T2.22)* `tokenroom suggest --json` is
  the provider-neutral friction signal Keyoku (the actuator) consumes — the documented seam
  (docs/EVOLVING-HARNESS.md "Where this lives"). tokenroom observes + reports; it does not
  evolve the harness.

> **Boundary (2026-06-12):** the evolution FRAMEWORK is Keyoku's, not tokenroom's —
> awareness vs. action. The items below read tokenroom's friction feed but are built in
...
> thinness fitness function), auto-eval (the gate to any autonomy). tokenroom stays the
